...
describe("diagnosticsApi", () => {
  ...
  it("uses only local report-id commands", async () => {
    ...
    await diagnosticsApi.openIssue("diagnostic-1");

    expect(invoke.mock.calls).toEqual([
      ["generate_diagnostic_report", { request }],
      [
        "export_diagnostic_report",
        { reportId: "diagnostic-1", path: "C:\\report.json" },
      ],
      ["open_diagnostic_issue", { reportId: "diagnostic-1" }],
    ]);
  });
});
